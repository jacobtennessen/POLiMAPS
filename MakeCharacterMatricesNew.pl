...
use vars qw( $opt_t $opt_v $opt_m $opt_o $opt_e $opt_r $opt_n $opt_x $opt_i);
...
Usage: perl MakeCharacterMatricesNew.pl options
 required:
 ...
 optional:
  ...
  -i  use all sites, including invariant sites [default = T (true), otherwise if F (false) only phylogenetically informative sites will be used]
";

#############

getopts('t:v:m:o:e:r:n:x:i:');
die $usage unless ($opt_t);
die $usage unless ($opt_v);
die $usage unless ($opt_m);

my ($filetable, $outgroupvcf, $matrixchroms, $output, $excludelist, $reqlist, $reqref, $nonmissingthresh, $useinvariant);
...
if (defined $opt_x) {
  $nonmissingthresh = $opt_x;
} else {
  $nonmissingthresh = $defaultnonmissingthresh;
}
...
foreach my $mc (@matrixchroms) {
    ...
    while (<OUTGROUP>) {
        ...
        if ($data[0] =~ /CHROM/gi) {
            ...
            next;
        } elsif (length($data[3]) > 1) {
            next;
        } elsif (length($data[4]) > 1) {
            next;
        }
        unless ($data[0] =~ /^$mc$/) {
            next;
        ...
        my $genopos;
        my $depthpos;
        my @formatdata = split ":", $data[8];
        for (my $f = 0; $f < (scalar(@formatdata)); $f++) {
            if ($formatdata[$f] =~ /GT/) {
               $genopos = $f; 
            } elsif ($formatdata[$f] =~ /DP/) {
               $depthpos = $f; 
            }
        }
        if (defined $genopos) {
          ...
          foreach my $i (@inds) {
              my @genodata = split ":", $i;
              my $depth;
              if ((defined $depthpos)&&($genodata[$depthpos] =~ /\d/)) {
                $depth = $genodata[$depthpos];
              } elsif ($genodata[$genopos] =~ /\d/) {
                $depth = 1;
              }
              if (($genodata[$genopos] =~ /1\/1/)&&(defined $depth)&&($depth > 0)) {
                  if (length($data[4]) == 1) {
                      $HoHoutgroup{$outgroups[$indcount]}{$data[1]} = $data[4];
                  }
              } elsif (($genodata[$genopos] =~ /^0$/)&&(defined $depth)&&($depth > 0)) {
                  $HoHoutgroup{$outgroups[$indcount]}{$data[1]} = $data[3];
              } elsif (($genodata[$genopos] =~ /0\/0/)&&(defined $depth)&&($depth > 0)) {
                  $HoHoutgroup{$outgroups[$indcount]}{$data[1]} = $data[3];
              }
              $indcount +=1;
          }
    ...
    foreach my $infosite (@infosites) {
        ...
        foreach my $o (@outgroups) {
            if (defined $HoHoutgroup{$o}{$chromdata[1]}) {
                if ($HoHoutgroup{$o}{$chromdata[1]} !~ /N/) {
                    $nonmissing +=1;
                    if (defined $bases{$HoHoutgroup{$o}{$chromdata[1]}}) {
                        $doublebases{$HoHoutgroup{$o}{$chromdata[1]}} = 1;
                    } else {
                        $bases{$HoHoutgroup{$o}{$chromdata[1]}} = 1;
                    }
                }            
            } else {
        ...
        if (($nonmissing > 1)&&(($useinvariant =~ /T/)||(((scalar (keys %doublebases)) >= 1)&&((scalar (keys %bases)) > 1)))) {
            my @tableline;
    ...
    foreach my $linkgroup (keys %HoAphylip) {
      if ((defined $nonmissing{$linkgroup})&&($nonmissing{$linkgroup} >= $nonmissingthresh)) {
        ...
        my @seqpieces;
        for (my $sp = 0; $sp < length($seq); $sp+= 100) {
          my $end = $sp + 99;
          unless (defined $HoAphylip{$linkgroup}[$end]) {
            $end = length($seq) - 1;
          }
          my $partialseq = join "", @{$HoAphylip{$linkgroup}}[$sp..$end];
          push @seqpieces, $partialseq;
        }
        ...
        push @fasta, ">$linkgroup.$nonmissing{$linkgroup}";
        push @fasta, @seqpieces;
      }
    }
    ...
    my $fastaoutputfile = "$output"."_$mc.fas";
    
    unless ( open(FAS, ">$fastaoutputfile") ) {
      print "Cannot open file \"$fastaoutputfile\" to write to!!\n\n";
      exit;
    }
    print FAS $fastaresult;
